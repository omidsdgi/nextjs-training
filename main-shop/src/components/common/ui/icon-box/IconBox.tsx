...
export function IconBox({icon,size=22,link,title,hideTitleOnMobile,badge=0}: Props) {
    ...
    return (
        <>
            ...
            {title && <div
                className={`${hideTitleOnMobile ? 'hidden xl:inline-block' : 'inline-block'}  text-medium text-gray-500 font-lato`}>{title}</div>}
        </>
    )
}