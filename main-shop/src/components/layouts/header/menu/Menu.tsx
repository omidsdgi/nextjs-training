        ...
        <>
            ...
            <nav id="main_menu">
                <ul className="flex flex-col lg:flex-row items-start lg:items-center text-heading6 lg:text-heading-sm 2xl:text-heading6 gap-[32px] mt-[32px] lg:mt-0 lg:gap-3 xl:gap-5 2xl:gap-10">
                        ...
                        {
                            menuMock.map((item: any, index: number) => {
                                return(
                                    <li>
                                        {

                                  item.icon ?  <IconBox key={index} {...item}  linkClassName={"flex flex-row gap-2 items-center"} size={24} titleClassName={"text-heading6 lg:text-heading-sm xl:text-heading6"} />:<Link href={item.link} className="flex flex-row">{item.title}</Link>
                            }
                                    </li>
                                    )
                        })
